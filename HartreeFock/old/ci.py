...
def CI_A(HFArchive, level):
    CI( HFArchive.N[0]//2, HFArchive.K,
        HFArchive.Hc, HFArchive.G, 
        HFArchive.C[0], HFArchive.Vnn, 
        level, HFArchive)

def CI(N, K, Hc, G, C, Vnn, level, HFArchive=None):
    t = timer()
    print('\n========== Begin CI ==========')
    CI_C = []
    exc_list = []
    ...
    for i in range(CI_K):
        for j in range(CI_K):
                ...
                #                 callback=__callback)
                CI_M[i,j] = __compute_ci_matrix_unit(i, j, CI_C, N, K, Hc, G, CI_M)
    # p.close()
    # p.join()
    t, dt = timer(), timer() - t
...
def __compute_ci_matrix_unit(n, m, CI_C, N, K, Hc, G, CI_M):
