...
def buildS(basis, S):
    ...
    for i in range(K):
        for j in range(i+1):
            ...
            for bi in Bi.cgf:
                for bj in Bj.cgf:
                    S[i][j] += bi.k * bj.k * overlap_integral(bi.exp, bj.exp, bi.R, bj.R, bi.ang, bj.ang)
            S[j][i] = S[i][j]
    return S
...
def buildT(basis, T):
    ...
    for i in range(K):
        for j in range(i+1):
            ...
            for bi in Bi.cgf:
                for bj in Bj.cgf:
                    T[i][j] += bi.k * bj.k * kinetic_energy_integral(bi.exp, bj.exp, bi.R, bj.R, bi.ang, bj.ang)
            T[j][i] = T[i][j]
    return T
            ...
            for bi in Bi.cgf:
                for bj in Bj.cgf:
                    for Z, _x, _y, _z in atoms:
                        ...
                        V[i][j] += bi.k * bj.k * nuclear_attraction_integral(bi.exp, bj.exp, bi.R, bj.R, bi.ang, bj.ang, R3, Z)
            V[j][i] = V[i][j]
    return V
    ...
    for l in range(ang1[0]+ang2[0]+1):
        for r in range(int(l/2)+1):
            for i in range(int((l-2*r)/2)+1):
                vx = __vi(l, r, i, e1+e2, R1[0], R2[0], ang1[0], ang2[0], R3[0], P[0])

                for m in range(ang1[1]+ang2[1]+1):
                    for s in range(int(m/2)+1):
                        for j in range(int((m-2*s)/2)+1):
                            vy = __vi(m, s, j, e1+e2, R1[1], R2[1], ang1[1], ang2[1], R3[1], P[1])
            ...
            for lp in range(0,ang3[0]+ang4[0]+1):
                for rp in range(0,int(lp/2)+1):
                    for i in range(0,int((l+lp-2*r-2*rp)/2)+1):
                        gx = __gi(l,lp,r,rp,i, ang1[0],ang2[0],R1[0],R2[0],P[0],ep, ang3[0],ang4[0],R3[0],R4[0],Q[0],eq)
                                ...
                                for mp in range(0,ang3[1]+ang4[1]+1):
                                    for sp in range(0,int(mp/2)+1):
                                        for j in range(0,int((m+mp-2*s-2*sp)/2)+1):
                                            gy = __gi(m,mp,s,sp,j, ang1[1],ang2[1],R1[1],R2[1],P[1],ep, ang3[1],ang4[1],R3[1],R4[1],Q[1],eq)
...
def __S(e1, e2, R1, R2, ang1, ang2):
    ...
    for i in range(3):
        ...
        for k in range(0,int((ang1[i]+ang2[i])/2)+1):
            s += __ck(2*k, ang1[i], ang2[i], P[i]-R1[i], P[i]-R2[i]) * special.factorial2(2*k-1, exact=True) / (2*(e1+e2))**k
        S *= s
    return S
